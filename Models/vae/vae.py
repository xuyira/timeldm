...
class _Encoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        blocks = []
        # initial convolution
        blocks.append( nn.Conv1d(in_channels, n_channels, kernel_size=3, stride=1, padding=1))
        ...
        blocks.append( nn.Conv1d(block_in_ch, z_channels, kernel_size=3, stride=1, padding=1))
...
class Encoder(nn.Module):
    def __init__(self, seq_len, feat_dim, hidden_layer_sizes, latent_dim, **ignorekwargs):
        ...
        for M_out in hidden_layer_sizes:
            self.dense_layers.append(nn.Linear(input_dim, M_out))
            input_dim = M_out

        self.z_mean = nn.Linear(hidden_layer_sizes[-1], latent_dim)
        self.z_log_var = nn.Linear(hidden_layer_sizes[-1], latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_layer_sizes, seq_len, feat_dim,  **ignorekwargs):
        ...
        for M_out in reversed(hidden_layer_sizes):
            self.dense_layers.append(nn.Linear(input_dim, M_out))
            input_dim = M_out

        self.final_dense = nn.Linear(hidden_layer_sizes[0], seq_len * feat_dim)
...
class AutoencoderKL(nn.Module):
    def __init__(self, embed_dim: int, hparams) -> None:
        ...
        self.quant_conv_mu = nn.Linear(hparams["hidden_layer_sizes"][-1], hparams["latent_dim"],1)
        self.quant_conv_log_sigma = nn.Linear(hparams["hidden_layer_sizes"][-1], hparams["latent_dim"],1)
        
        self.post_quant_conv = nn.Linear(hparams["latent_dim"], hparams["hidden_layer_sizes"][-1], 1)
...
if __name__ == '__main__': 
    ...
    # print(hparams)
    model = AutoencoderKL(3, hparams).to(device)
    input = torch.randn((1, 64, 5)).to(device)
    N, T, D = input.shape
