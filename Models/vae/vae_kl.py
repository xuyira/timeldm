...
from Models.interpretable_diffusion.model_utils import Conv_MLP
...
def Normalize(in_channels):
    return nn.GroupNorm(num_groups=5, num_channels=in_channels, eps=1e-6, affine=True)
...
class Encoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        blocks = []
        # initial convolution
        blocks.append(Conv_MLP_IN(in_channels, n_channels))
        ...
        blocks.append(Conv_MLP_OUT(block_in_ch, z_channels) )
...
class Decoder(nn.Module):
    def __init__(
        ...
        **ignorekwargs,
        ...
        blocks = []
        # initial conv
        blocks.append(Conv_MLP_IN(z_channels, block_in_ch) )
        ...
        blocks.append(Normalize(block_in_ch))
        blocks.append( Conv_MLP_OUT(block_in_ch, out_channels))
        self.blocks = nn.ModuleList(blocks)
...
class AutoencoderKL(nn.Module):
    def __init__(self, embed_dim: int, hparams) -> None:
        ...
        self.quant_conv_sigma = torch.nn.Conv1d(hparams["z_channels"], embed_dim, 1)
    ...
    def encode(self, x):
        ...
        h = self.encoder(x)
        # print('h',h.shape)
        z_mu = self.quant_conv_mu(h.transpose(1, 2)).transpose(1, 2)
        # print('z_mu',z_mu.shape)
        z_sigma = self.quant_conv_sigma(h.transpose(1, 2)).transpose(1, 2)
    ...
    def decode(self, z):
        # print('z', z.shape)
        z = self.post_quant_conv(z.transpose(1, 2)).transpose(1, 2)
        # print('z', z.shape)
        dec = self.decoder(z)
        # print('dec', dec.shape)
        return dec
...
if __name__ == '__main__': 
    ...
    # print(hparams)
    model = AutoencoderKL(hparams["embed_dim"], hparams).to(device)
    input = torch.randn((10, 24, 5)).to(device)
